...
def main():
    ...
    args = parse_args()
    args.method = "iterative_process_of_elimination"
    ...
    for dataset in args.datasets:
        ...
        # iterative process of elimination: remove one option during each iteration.
        current_options = num_of_options
        while current_options > 1:
            logger.info(f"Current number of options: {current_options}.")
            if current_options == num_of_options:
                log_probs = avg_log_probs
                mask_strategy = args.mask_strategy_for_process_of_elimination
                current_options = current_options - 1
            else:
                log_probs = poe_avg_log_probs
                mask_strategy = "lowest_iter"
                current_options = current_options - 1

            # masks = compute_mask_process_of_elimination(avg_log_probs, args.mask_strategy_for_process_of_elimination)
            masks = compute_mask_process_of_elimination(log_probs, mask_strategy)
